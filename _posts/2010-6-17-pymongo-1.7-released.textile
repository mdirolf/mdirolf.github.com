...
The second call should look familiar to those who have used the MongoDB shell extensively, while the first is a little less verbose and has always been the PyMongo way of doing things. Since PyMongo was first implemented, there have been some new features added to the server which are best exposed through the <code>dict</code> interface, however, like specifying keys that we _don't_ want returned:
